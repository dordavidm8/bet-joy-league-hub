...
// GET /api/quiz/next — fetch a question the user hasn't answered yet
    ...
    const result = await pool.query(
      `SELECT q.*
       ...
       WHERE q.is_active = true
         AND q.id NOT IN (
           SELECT question_id FROM quiz_attempts WHERE user_id = $1
         )
       ORDER BY RANDOM()
       LIMIT 1`,
      [req.user.id]
...
router.post('/:id/answer', authenticate, async (req, res, next) => {
  ...
  if (!selected_option) return res.status(400).json({ error: 'selected_option is required' });
  ...
  try {
    ...
    const questionRes = await client.query(
      'SELECT * FROM quiz_questions WHERE id = $1 AND is_active = true',
      [req.params.id]
    );
    ...
    const alreadyAnswered = await client.query(
      'SELECT id FROM quiz_attempts WHERE user_id = $1 AND question_id = $2',
    ...
    await client.query(
      `INSERT INTO quiz_attempts (user_id, question_id, selected_option, is_correct, points_earned)
       VALUES ($1,$2,$3,$4,$5)`,
    ...
    if (isCorrect) {
      ...
      await client.query(
        `INSERT INTO point_transactions (user_id, amount, type, reference_id, description)
         VALUES ($1,$2,'quiz_won',$3,'Quiz correct answer')`,
